...
import { generateStorageKey, inferMediaKind, slugifyMediaValue } from './mediaContracts';
const app = new Hono();
app.use('*', cors());
app.get('/health', (c) => c.json({ ok: true, service: 'awcms-edge' }));
const getAuthedSupabase = (env, token) => createClient(env.VITE_SUPABASE_URL, env.VITE_SUPABASE_PUBLISHABLE_KEY, {
    global: { headers: { Authorization: `Bearer ${token}` } }
});
const getAdminSupabase = (env) => createClient(env.VITE_SUPABASE_URL, env.SUPABASE_SECRET_KEY);
const getR2S3Client = (env) => new S3Client({
    region: 'auto',
...
const headStoredObject = (env, storageKey) => {
...
const getStoredObject = (env, storageKey) => {
    const client = getR2S3Client(env);
...
const ensureR2SigningConfig = (env) => {
...
const getSessionBoundAccessWindowSeconds = (env, token, requestedMaxAgeSeconds) => {
...
const buildStorageKey = (tenantId, fileName, folder, sessionBoundAccess = false) => {
...
const buildMediaSlug = (fileName, sessionId) => {
    const base = slugifyMediaValue(fileName) || 'media-item';
    return `${base}-${sessionId.slice(0, 8)}`;
};
const buildPublicMediaUrl = (requestUrl, storageKey) => {
...
const getUserContext = async (env, userId) => {
...
const resolveTenantId = (requestedTenantId, userContext) => {
...
app.get('/api/media/file/:id/access', async (c) => {
    ...
    if (media.session_bound_access) {
        ...
        ensureR2SigningConfig(c.env);
        const signedUrl = await getSignedUrl(getR2S3Client(c.env), new GetObjectCommand({
            Bucket: c.env.R2_BUCKET_NAME,
...
const handleMailketing = async (c) => {
    ...
    switch (action) {
        case 'send':
            ...
            if (body.attach1)
                params.attach1 = body.attach1;
            if (body.attach2)
                params.attach2 = body.attach2;
            if (body.attach3)
                params.attach3 = body.attach3;
            break;
        case 'subscribe':
    ...
    try {
        ...
        if (action === 'submit_application') {
            ...
            if (!verification.success)
                return c.json({ error: 'Security check failed' }, 400);
            if (!email || !full_name)
            ...
            if (existingRequest && existingRequest.status !== 'rejected')
                return c.json({ error: 'Application already pending for this email' }, 400);
        ...
        if (requestingUser) {
            ...
            const r = Array.isArray(userData.role) ? userData.role[0] : userData.role;
            roleName = r.name;
        ...
        switch (action) {
            ...
            case 'create': {
                ...
                if (!isSuperAdmin) {
                    ...
                    if (tenant_id && tenant_id !== requesterTenantId)
                        return c.json({ error: 'Forbidden: Cannot create user for another tenant' }, 403);
                ...
                if (createError)
                    return c.json({ error: 'Failed to create user: ' + createError.message }, 500);
            ...
            case 'invite': {
                ...
                if (!isSuperAdmin) {
                    if (!requesterTenantId)
                        return c.json({ error: 'Forbidden: no tenant context' }, 403);
                    if (tenant_id && tenant_id !== requesterTenantId)
                        return c.json({ error: 'Forbidden: Cannot invite user to another tenant' }, 403);
                    tenant_id = requesterTenantId;
                }
                ...
                if (inviteError)
                    return c.json({ error: 'Failed to invite user: ' + inviteError.message }, 500);
            ...
            case 'update': {
                ...
                if (!isSuperAdmin) {
                    ...
                    if (target && target.tenant_id !== requesterTenantId)
                        return c.json({ error: 'Forbidden' }, 403);
                }
