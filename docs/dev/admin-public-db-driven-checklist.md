...
This checklist aligns `docs/security/abac.md`, `docs/RESOURCE_MAP.md`, and `docs/architecture/standards.md` with a database-driven Admin UI/UX that controls all public tenants (`awcms-public/primary`, `awcms-public/primary`).
...
- [x] **Public (primary)**:
  - [x] `awcms-public/primary/src/lib/api.ts` -> add menu helper (menus table + tenant slug).
  - [x] `awcms-public/primary/src/components/Header.astro` -> use DB menu tree with JSON fallback.
  - [x] `awcms-public/primary/src/components/Footer.astro` -> use DB footer groups with JSON fallback.
...
- [x] **Public (primary)**: replace `src/data/blogs/*.json` with DB-backed reads (blogs/announcements). Production reads are DB-backed; local JSON fallback is development-only.
...
- [ ] **Public (primary)**: replace remaining image fallbacks with supported media sources.
...
- [x] **Public (primary)**: replace `src/data/site.json` and `contact.json` with settings data.
...
- [x] **Public (primary)**: fixed `TENANT_SLUG` in `src/lib/api.ts`.
...
- [ ] **Public (primary)**: middleware logging if SSR/runtime is enabled.
