...
- Cloudflare serves the public frontend and reviewed edge protections.
- The Hono backend API runs on the Coolify-managed VPS.
- PostgreSQL (`sikesrakobar`) runs as a protected dependency on the same VPS.
- Coolify manages the API service, PostgreSQL host lifecycle, and related operator environment.
- PostgreSQL remains reachable through the reviewed Hono backend path only.
...
- Store `TURNSTILE_SECRET_KEY` as a backend deployment-managed secret.
- For the reviewed SIKESRA hostname, Turnstile hostname validation should allow only `sikesrakobar.ahlikoding.com`.
- Store `EDGE_API_JWT_SECRET` as a backend deployment-managed secret.
- Keep deployed runtime secrets in reviewed deployment-managed secret storage, not in local env files.
