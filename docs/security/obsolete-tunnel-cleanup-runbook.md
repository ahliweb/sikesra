...
- confirming no active runtime still depends on Tunnel or Hyperdrive for PostgreSQL
...
- Database path: no Cloudflare Tunnel, no Hyperdrive in the active runtime path.
- References:
...
- Verify current docs and runtime config still define the no-Tunnel/no-Hyperdrive posture.
...
- Runtime smoke tests still pass without Tunnel/Hyperdrive assumptions:
...
- no active runtime path depends on Cloudflare Tunnel or Hyperdrive for PostgreSQL
